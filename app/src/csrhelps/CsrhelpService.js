(function () {
  ...
  function CsrhelpService($q) {
    ...
    function make_openssl(certf, context) {
      ...
      if (certf.country) openssl += "/C=" + certf.country;

      if (certf.state) openssl += "/ST=" + certf.state;

      if (certf.city) openssl += "/L=" + certf.city;

      if (certf.organization) openssl += "/O=" + certf.organization;

      if (certf.organization_unit) openssl += "/OU=" + certf.organization_unit;
    ...
    function make_exchange2007(certf, context) {
      ...
      if (certf.state && temp) exchange += ",st=" + certf.state;
      else if (certf.state && !temp) {
        exchange += "st=" + certf.state;
        temp = true;
      }

      if (certf.city && temp) exchange += ",l=" + certf.city;
      else if (certf.city && !temp) {
        exchange += "l=" + certf.city;
        temp = true;
      }

      if (certf.organization && temp) exchange += ",o=" + certf.organization;
      ...
      if (temp) exchange += ",cn=" + certf.hostname + '" ';
      else exchange += "cn=" + certf.hostname + '" ';
    ...
    function make_exchange2010(certf, context) {
      var exchange =
        'Set-Content -path ".\\' +
        getFilename(certf.hostname) +
        '.csr" -Value (New-ExchangeCertificate -GenerateRequest -KeySize ' +
        certf.keysize +
        ' -SubjectName "';
      var temp = false;
    ...
    function make_keytool(certf, context) {
      var keytool =
        "keytool -genkeypair -alias server -keyalg " +
        certf.algorithm +
        " -sigalg SHA256withRSA -keysize " +
        certf.keysize +
        " -keystore " +
        getFilename(certf.hostname) +
        '.jks -dname "';
      ...
      if (certf.algorithm == "ECC")
        keytool =
          "keytool -genkeypair -alias server -keyalg " +
          certf.algorithm +
          " -sigalg SHA256withECDSA -keysize " +
          getECCKeysize("keytool", certf.keysize) +
          " -keystore " +
          getFilename(certf.hostname) +
          '.jks -dname "';
      ...
      if (certf.state && temp) keytool += ",ST=" + certf.state;
      else if (certf.state && !temp) {
        keytool += "ST=" + certf.state;
        temp = true;
      }

      if (certf.city && temp) keytool += ",L=" + certf.city;
      else if (certf.city && !temp) {
        keytool += "L=" + certf.city;
        temp = true;
      }

      if (certf.organization && temp) keytool += ",O=" + certf.organization;
      else if (certf.organization && !temp) {
      ...
      if (temp) keytool += ",CN=" + certf.hostname + '" ';
      else keytool += "CN=" + certf.hostname + '" ';
    ...
    function make_bigip(certf, context) {
      ...
      if (certf.algorithm == "ECC")
        bigip =
          "create sys crypto key " +
          getFilename(certf.hostname) +
          " key-type ec-private curve-name " +
          getECCKeysize("bigip", certf.keysize) +
          " gen-csr";

      if (certf.country) bigip += ' country "' + certf.country + '"';

      if (certf.state) bigip += ' state "' + certf.state + '"';

      if (certf.city) bigip += ' city "' + certf.city + '"';
    ...
    function make_iis(certf, context) {
      ...
      if (certf.state && temp) iis += ",ST=" + certf.state;
      else if (certf.state && !temp) {
        iis += "ST=" + certf.state;
        temp = true;
      }

      if (certf.city && temp) iis += ",L=" + certf.city;
      else if (certf.city && !temp) {
        iis += "L=" + certf.city;
        temp = true;
      }

      if (certf.organization && temp) iis += ",O=" + certf.organization;
      else if (certf.organization && !temp) {
      ...
      if (temp) iis += ",CN=" + certf.hostname + '">>csrparams.inf\r\n';
      else iis += "CN=" + certf.hostname + '">>csrparams.inf\r\n';

      if (certf.algorithm == "ECC")
        iis +=
          'echo KeyAlgorithm="' +
          getECCKeysize("iis", certf.keysize) +
          '" >>csrparams.inf\r\n' +
          "echo KeyLength=" +
          getECCKeysize("keytool", certf.keysize) +
          " >>csrparams.inf\r\n";
    ...
    function make_any(certf, context) {
      ...
      sequence = sequence.then(function () {
        // Set hash algorithm
        var algorithm = org.pkijs.getAlgorithmParameters(
      ...
      sequence = sequence
        .then(function (result) {
          return crypto.digest(
            ...
            pkcs10_simpl.subjectPublicKeyInfo.subjectPublicKey.value_block
              .value_hex,
          );
        })
        .then(function (result) {
          pkcs10_simpl.attributes.push(
            new org.pkijs.simpl.ATTRIBUTE({
              type: "1.2.840.113549.1.9.14", // pkcs-9-at-extensionRequest
              values: [
                new org.pkijs.simpl.EXTENSIONS({
                  extensions_array: [
      ...
      sequence.then(
        function (result) {
          ...
          result_string =
            result_string +
            formatPEM(window.btoa(arrayBufferToString(pkcs10_encoded)));
          result_string =
      ...
      sequence.then(
        function (result) {
          var private_key_string = String.fromCharCode.apply(
            null,
            new Uint8Array(result),
          );
          var result_string = "\r\n-----BEGIN PRIVATE KEY-----\r\n";
          result_string =
            result_string + formatPEM(window.btoa(private_key_string));
    ...
    function stringToArrayBuffer(str) {
      ...
      for (var i = 0; i < stringLength; i++) resultView[i] = str.charCodeAt(i);
