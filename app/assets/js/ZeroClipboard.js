...
(function (window, undefined) {
  ...
  var _eventMessages = {
    ...
    error: {
      ...
      "flash-insecure":
        "Flash will be unable to communicate due to a protocol mismatch between your `swfPath` configuration and the page",
      "flash-unavailable":
      ...
      "browser-unsupported":
        "The browser does not support the required HTML DOM and JavaScript features",
    },
  };
  /**
   ...
   * @private
   */
  var _flashStateErrorNameMatchingRegex = new RegExp(
    "^flash-(" +
   ...
   * @private
   */
  var _flashStateEnabledErrorNameMatchingRegex = new RegExp(
    "^flash-(" +
      _flashStateErrorNames
  ...
  var _config = function (options) {
    if (typeof options === "object" && options && !("length" in options)) {
      _keys(options).forEach(function (prop) {
        if (
          /^(?:forceHandCursor|title|zIndex|bubbleEvents|fixLineEndings)$/.test(
            prop,
          )
        ) {
  ...
  var _state = function () {
    ...
    return {
      browser: _extend(
        _pick(_navigator, ["userAgent", "platform", "appName", "appVersion"]),
        {
    ...
    if (events && events.length && listener) {
      ...
      if (added.error) {
        ...
        for (i = 0, len = _flashStateErrorNames.length; i < len; i++) {
          if (
            _flashState[_flashStateErrorNames[i].replace(/^flash-/, "")] ===
            true
          ) {
            ZeroClipboard.emit({
              type: "error",
        ...
        if (
          ...
          ZeroClipboard.version !== _zcSwfVersion
        ) {
          ZeroClipboard.emit({
            type: "error",
  ...
  var _create = function () {
    ...
    if (
      _flashState.sandboxed !== previousState &&
  ...
  var _preprocessEvent = function (event) {
    ...
    switch (event.type) {
      case "error":
        var isSandboxed =
          event.name === "flash-sandboxed" ||
          _getSandboxStatusFromErrorEvent(event);
  ...
  var _vars = function (options) {
    ...
    if (typeof options.jsVersion === "string" && options.jsVersion) {
      str +=
        (str ? "&" : "") +
        "jsVersion=" +
        _encodeURIComponent(options.jsVersion);
    }
    return str;
  };
  /**
  ...
  var _getElementPosition = function (el) {
    ...
    if (el.getBoundingClientRect) {
      ...
      pos.height =
        "height" in elRect ? elRect.height : elRect.bottom - elRect.top;
    }
    return pos;
  };
  /**
      ...
      if (frame && frame.nodeType === 1 && frame.nodeName === "IFRAME") {
        ...
        if (
          ...
          (frameError &&
            ...
            /(^|[\s\(\[@])sandbox(es|ed|ing|[\s\.,!\)\]@]|$)/.test(
              frameError.message.toLowerCase(),
            ))
        ) {
          isSandboxed = true;
        }
   ...
   * If no `format` is provided, a copy of ALL pending data formats will be returned.
   *
   * @returns `String` or `Object`ZeroClipboard
   */
